...
- **Tutor filter dropdown:** the STAFF-only tutor filter is a custom dropdown (button + popover list, `#tutorFilterBtn`/`#tutorFilterMenu` in `calendar.ejs`, built by `setupTutorFilter()`) rather than a native `<select>` - a plain `<option>` can't mix a colored icon with separately-colored text. Each row shows a small square in that tutor's color (same rule as above: blue for the logged-in tutor, palette color otherwise) next to their name, in the theme's normal text color (`text-foreground` - originally hardcoded white, unreadable in light theme, fixed in 2.3.2). The popover is positioned `absolute` with `z-30`, above the calendar grid's sticky day-header row (`z-20`) so it doesn't render underneath it. `filterEventsByTutor()` applies the selection to both event types - a prenotation matched against its assigned tutor, a note against its creator (2.3.5) - so picking a tutor scopes their notes along with their prenotations.
...
**Fetching notes by creator, not just by assignment (GENERIC/GUEST):** a tutor who created a note for someone else, without also assigning it to themselves, still needs to see that note on their own calendar. `calendarDataService.js`'s shared range fetch (see [Calendar Performance: On-Demand Per-Week Loading](#calendar-performance-on-demand-per-week-loading) below) filters the date-bounded note fetch to `note.tutors.some(t => t.id === tutorId) || note.creator?.id === tutorId` - assignment OR authorship. STAFF skips this filter entirely (see [STAFF: Every Tutor's Notes, Visibility Toggles](#staff-every-tutors-notes-visibility-toggles) below) and gets every note in range regardless of assignment or authorship.
...
### STAFF: Every Tutor's Notes, Visibility Toggles
...
This exclusion is applied per data source, not globally - `GET /api/reports/tutor-monthly-hours` (the Staff Panel's on-page monthly hours table, fetched by `staffPanel.js` separately from the page load) initially kept showing erased tutors even after `/staffPanel`'s own page-load `tutors` list was fixed above - each data source needed the same `anonymizedAt` filter added on its own.
